...
class Point extends Object {
  ...
  public double angleYZ(double yAdj, double zAdj) {
    return Math.atan2(y - yAdj, zAdj - z);
  }
  ...
  public double angleXZ(double xAdj, double zAdj) {
    return Math.atan2(x - xAdj, zAdj - z);
  }
} // end class Point
...
class Dir extends Point {
  ...
  public Dir(double x, double y, double z) {
    setDir(x, y, z);
  }
...
class Interp {
  ...
  public void setXY(int i, double v, double w) {
    x[i] = v;
    y[i] = w;
  }
  ...
  public double interp4P(double v) {
    ...
    int imin, imax;
    i = 0;
    while (x[i] < v && i < x.length - 1) {
      i++;
    }
    if (i <= 1) {
      imin = 0;
      imax = Math.min(2, x.length - 1);
    } else if (i > x.length - 2) {
      imin = Math.max(0, x.length - 3);
      imax = x.length - 1;
    } else {
      imin = Math.max(0, i - 2);
      imax = Math.min(imin + 3, x.length - 1);
    }
    t = 0;
    for (i = imin; i <= imax; i++) {
      p = 1;
