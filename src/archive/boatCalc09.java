...
public boatCalc() {
   ...
   protected void paintComponent(Graphics g) {
    super.paintComponent(g);
    g.drawString(Double.toString(hull.base),10,10);
    
    g.drawString(Double.toString(pRdr.rudder.getParamX(0)),20,40);
    g.drawString(Double.toString(pRdr.rudder.getParamY(0)),20,60);
    
    g.drawString(Double.toString(pRdr.rudder.getParamX(1)),20,100);
    g.drawString(Double.toString(pRdr.rudder.getParamY(1)),20,120);

    g.drawString(Double.toString(pRdr.rudder.getParamX(2)),20,160);
    g.drawString(Double.toString(pRdr.rudder.getParamY(2)),20,180);

    g.drawString(Double.toString(pRdr.rudder.getParamX(3)),20,220);
    g.drawString(Double.toString(pRdr.rudder.getParamY(3)),20,240);
   ...
   protected void setTable() {
    ...
    double wy = 0;
      if(pRdr.rudder.use){
         jlRArea.setText(bcf.DF1d.format(hull.units.coefArea()*pRdr.rudder.getArea())+hull.units.lblArea());
         s = bcf.DF1d.format(pRdr.rudder.getAreaX())+", "+bcf.DF1d.format(pRdr.rudder.getAreaY());
         jlRCoA.setText(s);
         wa = wa + pRdr.rudder.getArea();
         wx = wx + pRdr.rudder.getArea() * pRdr.rudder.getAreaX();
         wy = wy + pRdr.rudder.getArea() * pRdr.rudder.getAreaY();
      }
      else{
      ...
      if(pRdr.skeg.use){
         jlSArea.setText(bcf.DF1d.format(hull.units.coefArea()*pRdr.skeg.getArea())+hull.units.lblArea());
         s = bcf.DF1d.format(pRdr.skeg.getAreaX())+", "+bcf.DF1d.format(pRdr.skeg.getAreaY());
         jlSCoA.setText(s);
         wa = wa + pRdr.skeg.getArea();
         wx = wx + pRdr.skeg.getArea() * pRdr.skeg.getAreaX();
         wy = wy + pRdr.skeg.getArea() * pRdr.skeg.getAreaY();
      }
      else{
   ...
   public void actionPerformed(ActionEvent e){
    ...
    try {d = Double.parseDouble(tfInc.getText());
    } catch (NumberFormatException nfe){
   ...
   public Component getComponentAfter(Container focusCycleRoot, Component aComponent){
      ...
      else if (aComponent.equals(p.rbTackZ)) return p.rbLuff;
      else if (aComponent.equals(p.rbLuff)) return p.rbBoom;
      else if (aComponent.equals(p.rbBoom)) return p.rbGaff;
      else if (aComponent.equals(p.rbGaff)) return p.rbAll;
      else if (aComponent.equals(p.rbAll)) return p.tfInc;
   ...
   public Component getComponentBefore(Container focusCycleRoot, Component aComponent){
      ...
      else if (aComponent.equals(p.rbL)) return p.tfInc;
      else if (aComponent.equals(p.tfInc)) return p.rbAll;
      else if (aComponent.equals(p.rbAll)) return p.rbGaff;
      else if (aComponent.equals(p.rbGaff)) return p.rbBoom;
      else if (aComponent.equals(p.rbBoom)) return p.rbLuff;
      else if (aComponent.equals(p.rbLuff)) return p.rbTackZ;
   ...
   protected void paintComponent(Graphics g) {
    super.paintComponent(g);
     ...
     if (ts.use){
        ...
        iw = xb + (int) (r * (ts.getX(Sail.THROAT) - pRig.getMinX()));
        iz = yb - (int) (r * (ts.getY(Sail.THROAT) - pRig.getMinY()));   
        g.drawLine(iu,iv,iw,iz);
        ...
        cx = xb + (int) (r * (ts.getAreaX() - pRig.getMinX()));
        cy = yb - (int) (r * (ts.getAreaY() - pRig.getMinY()));
     ...
     if (a > 0){
        cx = xb + (int) (r * (pRig.getAreaX() - pRig.getMinX()));
        cy = yb - (int) (r * (pRig.getAreaY() - pRig.getMinY()));
   ...
   JRadioButton rbTackX, rbTackZ, rbLuff, rbBoom, rbGaff, rbAll;
   JTextField tfInc;
   editSail pMain;
   editSail pJib;
   editSail pMiz;
   ...
   public sailData(int x, int y) {
      ...
      rbTackX = new JRadioButton("Tack X");
      rbTackZ = new JRadioButton("Tack Z");
      rbLuff = new JRadioButton("Luff");
      rbBoom = new JRadioButton("Foot");
      rbGaff = new JRadioButton("Head");
      rbAll = new JRadioButton("Scale (%)");
...
//   add(new Box.Filler(new Dimension(20,20),new Dimension(20,20),new Dimension(20,20)));
 ...
 //  add(new Box.Filler(new Dimension(20,20),new Dimension(20,20),new Dimension(20,20)));
   ...
   public pnlAbout(){
      ...
      lbl.setHorizontalAlignment(SwingConstants.CENTER);
      add(lbl);
      lbl = new JLabel ("Version 0.2d - 02/16/2004");
      lbl.setFont(wpFont);
      lbl.setHorizontalAlignment(SwingConstants.CENTER);
      add(lbl);
   }
